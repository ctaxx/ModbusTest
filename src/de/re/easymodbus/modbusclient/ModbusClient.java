...
public class ModbusClient {
    ...
    public ModbusClient(String ipAddress, int port) {
        ...
        if (debug) {
            StoreLogData.getInstance().Store("EasyModbus library initialized for Modbus-TCP, IPAddress: " + ipAddress + ", Port: " + port);
        }
    ...
    /**
     * Convert two 16 Bit Registers to 64 Bit double value Reg0: Low
     * Word.....Reg3: High Word
     *
     * @param registers 16 Bit Registers
    ...
    /**
     * Convert two 16 Bit Registers to 64 Bit double value Order "LowHigh":
     * Reg0: Low Word.....Reg3: High Word, "HighLow": Reg0: High Word.....Reg3:
     * Low Word
     *
     * @param registers 16 Bit Registers
    ...
    /**
     * Convert four 16 Bit Registers to 64 Bit long value Reg0: Low
     * Word.....Reg3: High Word
     *
     * @param registers 16 Bit Registers
     * @return 64 bit value
    ...
    /**
     * Convert four 16 Bit Registers to 64 Bit long value Register Order
     * "LowHigh": Reg0: Low Word.....Reg3: High Word, "HighLow": Reg0: High
     * Word.....Reg3: Low Word
     *
     * @param registers 16 Bit Registers
     * @return 64 bit value
     ...
     * easymodbusclient/discreteinputs/'address' e.g.
     * easymodbusclient/discreteinputs/0 for address "0". Note that the Address
     * that will be publishes is "0"-Based. The Root topic can be changed using
     * the Parameter By default we are using the Standard-Port 1883. This Port
     * can be changed using the Property "MqttBrokerPort" A Username and
     * Passowrd can be provided using the Properties "MqttUserName" and
     * "MqttPassword" 'MqttRootTopic' Default is 'easymodbusclient'
     *
    ...
    /**
     * Read Coils from Server device (FC1) and publishes the values to a
     * MQTT-Broker. The Topic will be easymodbusclient/coils/'address' e.g.
     * easymodbusclient/coils/0 for address "0". Note that the Address that will
     * be publishes is "0"-Based. The Root topic can be changed using the
     * Parameter By default we are using the Standard-Port 1883. This Port can
     * be changed using the Property "MqttBrokerPort" A Username and Passowrd
     * can be provided using the Properties "MqttUserName" and "MqttPassword"
    ...
    /**
     * Read Holding Registers from Server device and publishes the values to a
     * MQTT-Broker. The Topic will be
     * easymodbusclient/holdingregisters/'address' e.g.
     * easymodbusclient/holdingregisters/0 for address "0". Note that the
     * Address that will be publishes is "0"-Based. The Root topic can be
     * changed using the Parameter By default we are using the Standard-Port
     * 1883. This Port can be changed using the Property "MqttBrokerPort" A
     * Username and Passowrd can be provided using the Properties "MqttUserName"
...
//        byte tmpByte = data[7];
//        System.err.println("data7=" + tmpByte);
//        System.err.println(" data8="+(byte)data[8]);
...
//            throw new de.re.easymodbus.exceptions.ModbusException("Error reading");
    ...
    /**
     * Read Input Registers from Server device and publishes the values to a
     * MQTT-Broker. The Topic will be easymodbusclient/inputregisters/'address'
     * e.g. easymodbusclient/inputregisters/0 for address "0". Note that the
     * Address that will be publishes is "0"-Based. The Root topic can be
     * changed using the Parameter By default we are using the Standard-Port
     * 1883. This Port can be changed using the Property "MqttBrokerPort" A
     * Username and Passowrd can be provided using the Properties "MqttUserName"
...
//        if (((int) (data[7] & 0xff)) == 0x90 & data[8] == 0x04) {
    ...
    public boolean isConnected() {
        ...
        if (tcpClientSocket == null) {
            returnValue = false;
        } else if (tcpClientSocket.isConnected()) {
            returnValue = true;
        } else {
            returnValue = false;
        }
    ...
    /**
     * True: Values will be published only on change - FALSE: after every
     * request
     *
     * @param mqttPushOnChange True: Values will be published only on change -
     * FALSE: after every request
     */
    ...
    /**
     * Disables or Enables to Retain the Messages in the Broker - default is
     * false (Enabled)
     *
    ...
    /**
     * Sets and enables the Logfilename which writes information about received
     * and send messages to File
     *
